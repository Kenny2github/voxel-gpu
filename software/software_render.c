...
void render_software() {
    ...
    int curr_voxel = 0;
    uint32_t* voxel_pointer = (uint32_t*)VOXEL_SPACE_START;
    while(curr_voxel++ < voxel_count) {
    
        uint8_t palette = *voxel_pointer & 0x3;
        uint16_t z = (*voxel_pointer >> 2) & 0x3FF;
        uint16_t y = (*voxel_pointer >> 12) & 0x3FF; 
        uint16_t x = (*voxel_pointer >> 22) & 0x3FF; 
            
        // printf("%d %d %d %d\n", palette, z, y, x);
        
        /* 
        Ray-casting method unoptimized:
        - Compute a ray per pixel
        - For each ray, check if it intersects the axis-aligned voxel
        - If it intersects, color pixel
        */
       // Rough instruction count: 104605090 per voxel
    //    {
    //         debug_start();
    //         for(int xs = 0; xs < H_RESOLUTION; xs++) {
    //             for(int ys = 0; ys < V_RESOLUTION; ys++) {
    //                 cameraRay.direction = add_vector(add_vector(topLeft, multiply_vector(horizontalVec, xs)), multiply_vector(verticalVec, V_RESOLUTION-1 - ys));
    //                 float t = check_box_intersection(x, y, z, &cameraRay);
    //                 if(t == -1)
    //                     continue;
    ...
    //    }
        ...
        /*
        Ray-casting method optimized:
        - Partitions the screen into multiple sections
        - For each section, checks if center-pixel ray hits the voxel 
        - If hit, flood-fill from the center pixel until the pixel does not hit the voxel
        - If no hit, partition again and go to step 2
        */
        // Rough instruction count: 6716143 per voxel (94% reduction from brute-force)
        // {
        //     debug_start();
        //     partition_and_fill(
